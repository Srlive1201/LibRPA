                   ...
                   TFGrids::GRID_TYPES gt, bool use_space_time)
{
    ...
    if ( gt == TFGrids::GRID_TYPES::Minimax )
    {
        double emin, emax;
        ...
        throw invalid_argument("chi0 on current grid is not implemented");
    ...
    if (use_space_time)
    {
        ...
        for (int iR = 0; iR < Rlist.size(); iR++)
            ...
            {
                // FIXME: can run, but result totally wrong!!!
...
void Chi0::build_gf_Rt(size_t iR, Vector3_Order<int> R, size_t itau, char ov)
{
    ...
    for (int is = 0; is != nspins; is++)
    {
        ...
        {
            double kphase = std::exp(- klist[ik] * (R * latvec) * TWO_PI);
