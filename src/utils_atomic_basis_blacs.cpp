...
/* ========================================================================= */
/* For matrix redistribution from atom-pair (AP) to BLACS block-cyclic forms */
/* ========================================================================= */
...
// For symmetric/Hermitian matrices case
...
/* ========================================================================= */
/* For matrix redistribution from BLACS block-cyclic to atom-pair (AP) forms */
/* ========================================================================= */
...
_get_communicate_ids_list_blacs_to_ap(const int &myid,
                                      ...
                                      const Array_Desc &ad, const bool row_fast, const bool return_local)
{
    ...
    // Objects to return
    // process ID -> list of basis indices (BLACS context), sending from myid to others
    std::map<int, std::vector<std::pair<size_t, size_t>>> map_proc_id2d_send;
    // process ID -> list of basis indices (atom-pair context), receiving by myid from others
    std::map<int, std::vector<std::pair<atpair_t, std::vector<std::pair<size_t, size_t>>>>> map_proc_id2d_recv;

    int myprow, mypcol;
    int dummy = 0;

    // basis indices required at myid in the atom-pair distribution
    const std::vector<atpair_t> &IJs_require =
        map_proc_IJs_require.count(myid) ? map_proc_IJs_require.at(myid) : std::vector<atpair_t>();
    const auto ids_ap = get_2d_mat_indices_atpair(atbasis_r, atbasis_c, IJs_require, row_fast);

    // global basis indices available in BLACS
    const auto ids_blacs = get_2d_mat_indices_blacs(ad, myid, row_fast);

    std::map<int, std::map<atpair_t, std::vector<std::pair<size_t, size_t>>>> map_proc_map_id2d_recv;
    std::map<int, std::vector<atpair_t>> map_proc_IJs_save_order;
    for (const auto &id: ids_ap)
    {
        const auto &i_gl = as_int(id.first);
        ...
        if (myid != target)
        {
            if (return_local)
            {
                ...
                const atpair_t ij_lo = {static_cast<atom_t>(i_lo), static_cast<atom_t>(j_lo)};
                map_proc_map_id2d_recv[target][IJ].emplace_back(ij_lo);
            }
            else
            {
                // NOTE: global index requested, save all to the (0,0) dummy pair index
                ...
                map_proc_map_id2d_recv[target][IJ].emplace_back(id);
            }
        }
    }
    for (const auto &target_map: map_proc_map_id2d_recv)
    {
        const auto &target = target_map.first;
        const auto &IJs_save_order = map_proc_IJs_save_order.at(target);
        for (const auto &IJ: IJs_save_order)
        {
            map_proc_id2d_recv[target].push_back({IJ, target_map.second.at(IJ)});
        }
    }
    ...
    for (const auto &id: ids_blacs)
    {
        ...
        if (map_IJs_proc.count(IJ))
        {
            ...
            if (return_local)
            {
                ...
                const auto i_lo = as_size(ad.indx_g2l_r(as_int(id.first)));
                const auto j_lo = as_size(ad.indx_g2l_c(as_int(id.second)));
                map_proc_id2d_send[source].push_back({i_lo, j_lo});
            }
            else
            {
                map_proc_id2d_send[source].emplace_back(id);
            }
        }
    }
                                            ...
                                            const Array_Desc &ad, bool row_fast, bool row_major)
{
    ...
    const auto ids = _get_communicate_ids_list_blacs_to_ap(myid, map_proc_IJs_require, atbasis_r,
                                                           atbasis_c, ad, row_fast, false);
