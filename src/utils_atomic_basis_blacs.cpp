...
std::pair<std::map<int, std::vector<std::pair<atpair_t, std::vector<std::pair<size_t, size_t>>>>>,
          std::map<int, std::vector<std::pair<size_t, size_t>>>>
_get_communicate_ids_list_ap_to_blacs_sy(std::map<int, std::vector<char>> &map_lconj,
                                          const int &myid,
                                          const char &uplo,
...
// indices computation backend for BLACS->AP conversion of symmetric matrix
// FIXME: not correctly working now
static
std::pair<std::map<int, std::vector<std::pair<size_t, size_t>>>,
          std::map<int, std::vector<std::pair<atpair_t, std::vector<std::pair<size_t, size_t>>>>>>
_get_communicate_ids_list_blacs_to_ap_sy(std::map<int, std::vector<std::pair<atpair_t, std::vector<char>>>> &map_lconj,
                                         const int &myid,
                                         const char &uplo,
                                         ...
                                         const bool return_local)
{
    ...
    // basis indices required at myid in the atom-pair distribution
    const std::vector<atpair_t> &IJs =
        map_proc_IJs_require.count(myid) ? map_proc_IJs_require.at(myid) : std::vector<atpair_t>();
    const auto ids_ap = get_2d_mat_indices_atpair(atbasis, atbasis, IJs, row_fast);
    ...
    std::map<int, std::map<atpair_t, std::vector<std::pair<size_t, size_t>>>> map_proc_map_id2d_ap;
    std::map<int, std::map<atpair_t, std::vector<char>>> map_lconj_IJ;
    std::map<int, std::vector<atpair_t>> map_proc_IJs_save_order;
    for (const auto &id: ids_ap)
    {
        const auto &i_gl = as_int(id.first);
    ...
    return {map_proc_id2d_blacs, map_proc_id2d_ap};
}
                                           ...
                                           const Array_Desc &ad, bool row_fast, bool row_major)
{
    ...
    std::map<int, std::vector<size_t>> map_proc_id1d_send;
    std::map<int, std::vector<std::pair<atpair_t, std::vector<size_t>>>> map_proc_id1d_recv;
...
/* ============================================================================================= */
/* For symmetric/Hermitian matrix redistribution from BLACS block-cyclic to atom-pair (AP) forms */
/* ============================================================================================= */
                                               ...
                                               const Array_Desc &ad, bool row_fast, bool row_major)
{
    ...
    std::map<int, std::vector<std::pair<atpair_t, std::vector<char>>>> map_lconj;
    const auto ids = _get_communicate_ids_list_blacs_to_ap_sy(map_lconj, myid, uplo, map_proc_IJs_avail, atbasis,
                                                              ad, row_fast, false);

    const auto &map_proc_id2d_send = ids.first;

    std::map<int, std::vector<std::pair<size_t, size_t>>> map_proc_id2d_recv;
    for (const auto &proc_id2d: ids.second)
    {
        const auto &proc = proc_id2d.first;
        // only up to 1 member for the case of global indices, which is for the the (0,0) dummy atom pair
        assert(proc_id2d.second.size() == 1);
    ...
    for (const auto &proc_data: map_proc_id2d_recv)
    {
        ...
        map_proc_id1d_lconj_recv[proc].first = flatten_2d_indices(id2d, atbasis.nb_total, atbasis.nb_total, row_major);
        size_t count = 0;
        for (const auto &IJ_lconj: map_lconj.at(proc))
        {
            count += IJ_lconj.second.size();
        }
        assert(count == id2d.size());
        std::vector<char> lconj(count);
        size_t disp = 0;
        for (const auto &IJ_lconj: map_lconj.at(proc))
        {
            std::copy(IJ_lconj.second.cbegin(), IJ_lconj.second.cend(), lconj.begin() + disp);
        }
        // flag for conjugate
        map_proc_id1d_lconj_recv[proc].second = std::move(lconj);
    }
                                              ...
                                              const Array_Desc &ad, bool row_fast, bool row_major)
{
    ...
    const auto ids = _get_communicate_ids_list_blacs_to_ap_sy(map_lconj, myid, uplo, map_proc_IJs_avail, atbasis,
                                                               ad, row_fast, true);
    // process ID -> list of basis indices (BLACS), sending from myid to others
    const auto &map_proc_id2d_send = ids.first;
    // process ID -> list of basis indices (AP), receiving by myid from others
    const auto &map_proc_id2d_recv = ids.second;

    // flatten 2D indices to 1D
    std::map<int, std::vector<size_t>> map_proc_id1d_send;
    std::map<int, std::vector<std::pair<atpair_t, std::pair<std::vector<size_t>, std::vector<char>>>>> map_proc_id1d_lconj_recv;

    for (const auto &proc_data: map_proc_id2d_send)
    {
        const auto &proc = proc_data.first;
        const auto &id2d = proc_data.second;
        map_proc_id1d_send[proc] = flatten_2d_indices(id2d, as_size(ad.m_loc()), as_size(ad.n_loc()), row_major);
    }

    for (const auto &proc_IJ_id2d: map_proc_id2d_recv)
    {
        const auto &proc = proc_IJ_id2d.first;
        for (size_t i = 0; i < proc_IJ_id2d.second.size(); i++)
        {
            const auto &IJ_id2d = proc_IJ_id2d.second[i];
            const auto &IJ = IJ_id2d.first;
            const auto &I = IJ.first;
            const auto &J = IJ.second;
            assert(IJ == map_lconj.at(proc)[i].first);
            const auto &lconj = map_lconj.at(proc)[i].second;
            auto &proc_vec = map_proc_id1d_lconj_recv[proc];
            const auto id1d = flatten_2d_indices(IJ_id2d.second, atbasis.get_atom_nb(I), atbasis.get_atom_nb(J), row_major);
            const std::pair<std::vector<size_t>, std::vector<char>> vpair{std::move(id1d), std::move(lconj)};
            const std::pair<atpair_t, std::pair<std::vector<size_t>, std::vector<char>>> v(IJ, std::move(vpair));
            proc_vec.emplace_back(v);
        }
    }
    return {map_proc_id1d_send, map_proc_id1d_lconj_recv};
