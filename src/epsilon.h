...
map<double, atom_mapping<std::map<Vector3_Order<double>, Matz>>::pair_t_old>
...
map<double, atom_mapping<std::map<Vector3_Order<int>, Matz>>::pair_t_old>
FT_Wc_freq_q(
    const map<double,
...
cplxdb compute_pi_det_blacs(ComplexMatrix &loc_piT, const LIBRPA::Array_Desc& arrdesc_pi, int *ipiv, int &info);

cplxdb compute_pi_det_blacs_2d(Matz &loc_piT, const LIBRPA::Array_Desc &arrdesc_pi, int *ipiv, int &info);
