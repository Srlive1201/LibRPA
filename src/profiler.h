...
struct Profiler
{
    ...
    static std::vector<int> timer_levels;
    //! Explanatory note of the timer
    static std::vector<std::string> timer_notes;
    //! Search the timer with requested timer name
    static Timer * find_timer(const char *tname);

    //! Add a timer
    static void add(int level, const char *tname, const char *tnote = "");
    //! Start a timer
    static void start(const char *tname);
    //! Stop a timer and record the timing
    static void stop(const char *tname);
    //! Display the current profiling result
    static void display();
    //! Get the number of created timers
    static int get_num_timers() { return timers.size(); };
};
