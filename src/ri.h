...
typedef atom_mapping< map<Vector3_Order<int>, std::shared_ptr<matrix>> >::pair_t_old apair_R_mat_t;
//! type alias of atom-pair mapping to complex matrix indexed by reciprocal vector
typedef atom_mapping< map<Vector3_Order<double>, std::shared_ptr<ComplexMatrix>> >::pair_t_old apair_k_cplx_mat_t;

//! Tri-coefficient of localized RI (LRI) in real space. ABF on the first atom of the atom pair
extern apair_R_mat_t Cs;
//! Coulomb matrix in ABF, represented in the reciprocal space.
extern apair_k_cplx_mat_t Vq;
