...
void task_scRPA_band()
{
    ...
    Profiler::start("scRPA_band", "scRPA_band quasi-particle calculation");
    ...
    * First load the information of k-points along the k-path */
    Profiler::start("scrpa_band_load_band_mf", "Read eigen solutions at band kpoints");
    ...
    Profiler::stop("scrpa_band_load_band_mf");
        ...
        if (mpi_comm_global_h.is_root())
        {
            ...
            for (int i_spin = 0; i_spin < meanfield_band.get_n_spins(); i_spin++)
            {
                for (int i_kpoint = 0; i_kpoint < meanfield_band.get_n_kpoints(); i_kpoint++)
                {
                    ...
                    Vc_all[i_spin][i_kpoint] = calculate_scRPA_exchange_correlation(meanfield_band,freq,f_weight,sigc_sk,G0_matrix,i_spin,i_kpoint,n_bands,meanfield_band.get_efermi(),temperature);
            ...
            for (int i_spin = 0; i_spin < meanfield_band.get_n_spins(); i_spin++)
            {
                ...
                for (int i_kpoint = 0; i_kpoint < meanfield_band.get_n_kpoints(); i_kpoint++)
                {
                    ...
                    for (int i_state = 0; i_state < meanfield_band.get_n_bands(); i_state++)
                    {
                        ...
                        ofs_hf << std::setw(15) << std::setprecision(5) << occ_state << std::setw(15) << std::setprecision(5) << std::real(eks_state.real()- vxc_state + exx_state);
